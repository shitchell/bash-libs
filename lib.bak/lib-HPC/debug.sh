...
function debug() {
    ...
    local debug_lines=()  # the message to print
    ...
    if [[
        ...
        if [[ "${#}" -eq 1 && ("${1}" == "-" || "${1}" == "/dev/stdin") ]]; then
            # if the message is "-" or "/dev/stdin", read from stdin
            readarray -t debug_lines < <(cat)
        else
            # otherwise, use the arguments as the message
            debug_lines=("${@}")
        fi
...
# @description Debug a message from stdin and then print it to stdout
# @usage echo "foo" | debug-pipe <message> | cat
function debug-pipe() {
    local debug_message
    debug_message=$(cat)
    debug "${@}" "${debug_message}"
...
function debug-vars() {
    local var_names=() var_name debug_message declare_str
